#!/usr/bin/env bash
# One formal Phase-aware trial selected from round two.
...
MATRIX="formal_round2_p10 4 4 2e-6 0.2 true"
exec "$SNN2_PYTHON" scripts/phase_aware_sweep.py \
  ...
  --phase-reference-config "${PHASE_CONTROL_CFG:-artifacts/phase_aware_formal_v1/configs/p03_g4_s4.0_lr1e-06_w0.2_clip1.yaml}" \
  --source-config "${SOURCE_CFG:-configs/generated/exp1_qwen3_8b_tldr__phase_aware.yaml}" \
  --output "${SWEEP_DIR:-artifacts/phase_aware_round2_formal_cliptrue_v3}" "$@"
