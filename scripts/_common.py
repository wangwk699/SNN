...
def parser(
    ...
    if neuron or deployment_overrides:
        ...
        result.add_argument("--phase-base", type=float, default=None)
...
def apply_deployment_overrides(args, cfg):
    ...
    phase_base = getattr(args, "phase_base", None)
    mtn_T = getattr(args, "mtn_T", None)
    mtn_K = getattr(args, "mtn_K", None)
    neuron = getattr(args, "neuron", None)
    if neuron in {"ann", "gif"} and any(value is not None for value in (phase_T, phase_base, mtn_T, mtn_K)):
    ...
    if neuron == "phase" and any(value is not None for value in (mtn_T, mtn_K)):
        raise ValueError("Phase deployment accepts only --phase-T/--phase-base")
    if neuron == "mtn" and any(value is not None for value in (phase_T, phase_base)):
    ...
    if phase_base is not None:
        ...
        cfg["phase"]["base"] = validate_phase_base(phase_base)
...
def setup(config_path: str, config_scope: str = "run"):
    ...
    elif config_scope == "post_finetuning_clip_profile":
        config_dir = layout.post_finetuning_clip_profile_config_dir
