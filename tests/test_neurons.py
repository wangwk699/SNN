...
def test_phase_supports_all_grouped_layouts(kind, shape):
    module = PhaseSurrogate(_phase_state(kind), T=4, base=2.0, surrogate_slope=1.0)
    x = torch.randn(*shape)
...
def test_phase_hard_forward_and_temporal_sum_match():
    module = PhaseSurrogate(_phase_state(), T=4, base=2.0)
    x = torch.randn(2, 3, 4)
...
def test_phase_rejects_wrong_head_or_parameter_shape():
    module = PhaseSurrogate(_phase_state("attention_head_grouped"), T=4, base=2.0)
    ...
    with pytest.raises(ValueError, match="tau shape"):
        PhaseSurrogate(state, T=4, base=2.0)
...
def test_sequential_phase_runtime_t_mismatch_is_rejected_even_at_one():
    ...
    with pytest.raises(ValueError, match="provenance mismatch"):
        PhaseSurrogate(state, T=1, base=2.0)
